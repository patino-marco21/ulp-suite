...
describe('ulp-dedupe', () => {
  test('DEDUPE_BY is the content key (url, email, password)', () => {
    expect(DEDUPE_BY).toBe('url, email, password')
  })

  describe('dedupeLimitBy', () => {
    test('emits `LIMIT 1 BY <content key>` when deduping', () => {
      expect(dedupeLimitBy(true)).toBe('LIMIT 1 BY url, email, password')
    })
  ...
  describe('dedupeCountExpr', () => {
    test('counts distinct credentials via uniq() when deduping', () => {
      expect(dedupeCountExpr(true)).toBe('uniq(url, email, password)')
    })
