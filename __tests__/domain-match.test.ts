...
describe('buildCandidateColumnWhereClause', () => {
  ...
  test('the suffix param is dot-prefixed, so a domain merely ending with the same letters cannot false-match', () => {
    // eviltrezor.io must NOT be treated as a subdomain of trezor.io — the
    // dot boundary in the generated {domainSuffix0} param ('.trezor.io', not
    // 'trezor.io') is what SQL's endsWith() relies on for that. Same
    // dot-boundary guarantee domainMatches() enforces in JS (see the
    // 'does not match a different domain that merely ends with the same
    // letters' case above) — this asserts the SQL side agrees.
    const { params } = buildCandidateColumnWhereClause('domain', ['trezor.io'])
    expect(params.domainSuffix0).toBe('.trezor.io')
  })

  test('both domain and email_domain use the identical endsWith shape — both are covered by an ngram skip index (DDL v19 / design doc §1)', () => {
    const domainClause = buildCandidateColumnWhereClause('domain', ['trezor.io']).clause
    const emailClause = buildCandidateColumnWhereClause('email_domain', ['trezor.io']).clause
    expect(domainClause).toContain('endsWith(domain, ')
    expect(emailClause).toContain('endsWith(email_domain, ')
    expect(domainClause).not.toContain('reverse')
    expect(emailClause).not.toContain('reverse')
  })
})
